 ...
 *
 *   node build_flowchart.js
 */
import { fileURLToPath } from 'node:url';
import path from 'node:path';

import { createDocument, placeBox, applyPen, validate, exportSvg, saveDocument } from './src/core/index.js';
...
placeBox(doc, 'base', { id: 'start', at: 'AT5', span: '30x6', label: 'Start', shape: 'terminator', align: 'center', fill: INK.start });

for (const [id, row, label] of SPINE) {
  placeBox(doc, 'base', { id, at: `AT${row}`, span: '30x9', label, shape: 'decision', align: 'center', fill: INK.decision });
}
...
placeBox(doc, 'base', { id: 'congrats', at: 'AT132', span: '30x7', label: 'Congrats! You have a beautiful diagram.', shape: 'terminator', align: 'center', fill: INK.end });
...
const YES_ROWS = [24, 37, 50, 63, 76, 89, 102, 115, 128];
...
await saveDocument(doc, out('flowchart-important-process.turtlepen.json'));
